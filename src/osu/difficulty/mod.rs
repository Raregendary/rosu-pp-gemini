...
impl DifficultyValues {
    pub fn calculate(difficulty: &Difficulty, map: &Beatmap) -> Self {
        ...
        let great_hit_window = 2.0 * map_attrs.hit_windows().od_great.unwrap_or(0.0);
        let preempt = map_attrs.hit_windows().ar.unwrap_or(0.0);
    ...
    pub fn eval(attrs: &mut OsuDifficultyAttributes, mods: &GameMods, skills: &OsuSkills) {
        ...
        let slider_factor = if aim_difficulty_value > 0.0 {
            calculate_aim_difficulty_rating(aim_no_sliders_difficulty_value)
                / calculate_aim_difficulty_rating(aim_difficulty_value)
        } else {
            1.0
        };
        ...
        let base_flashlight_performance = 25.0 * flashlight_rating.powi(2);
        ...
        attrs.speed_note_count = speed.relevant_object_count();
    }
...
pub fn calculate_aim_difficulty_rating(difficulty_value: f64) -> f64 {
